...
def schedule(request):
    if request.method == 'POST':
        ...
        try:
            schedule_html = Class.objects.get(
                name=selected_class).schedule_html
...
def check_available_classrooms(request):
    ...
    for class_ in classes:
        ...
        occupied_classrooms_json = functions.parse_str_occupied_classrooms_to_json(
            occupied_classrooms_str)
...
def check_classroom_availability(request):
    ...
    context = {'form': form}
    if classroom:
        classroom_availability_in_the_week = functions.classroom_availability_in_the_week(
            classroom)
...
def update_schedules(request):
    ...
    return render(request, 'schedules_list.html', {'form': ChooseScheduleForm(), 'done': 'Schedules Updated!'})
